...
(function(Fucksvg){
	...
	function DataBox(name, canvas) {
		...
		this.messageBus = new Fucksvg.util.MessageBus();
	...
	DataBox.prototype.findCloserNode = function (node, cond){
		...
		for (var i = this.nodes.length-1; i >=0 ; i--) {
			...
			if(cond(typeNode) == true){
				var dist = Fucksvg.util.getDistance(node, typeNode);
	...
	DataBox.prototype.mousedown = function(event){
		var box = this;
		var xy = Fucksvg.util.getXY(box, event);
		var x = xy.x;
		var y = xy.y;
	...
	DataBox.prototype.mouseup = function(event){
		var box = this;
		var xy = Fucksvg.util.getXY(this, event);
		var x = xy.x;
		var y = xy.y;
		var dx = (x - box.startDragMouseX);
		var dy = (y - box.startDragMouseY);
	...
	DataBox.prototype.add = function (e) {
		...
		if(e instanceof Fucksvg.Container) {
			this.containers.push(e);
		}else if (e instanceof Fucksvg.Link) {
			this.links.push(e);
		}else if (e instanceof Fucksvg.Node) {
			this.nodes.push(e);
		}
		this.elementMap[e.id] = e;
	};
	...
	DataBox.prototype.adjustPosition = function(node){
		...
		var points = null;
		if(type == 'star'){
			points = Fucksvg.Layout.getStarPositions(node.x, node.y, childs.length, node.layout.radius,
												   node.layout.beginDegree, node.layout.endDegree);
		}else if(type == 'tree'){
			points = Fucksvg.Layout.getTreePositions(node.x, node.y, childs.length, layout.width,
