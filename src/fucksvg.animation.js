...
(function(Fucksvg){
	...
	function dividedTwoPiece(node, option){
		...
		function split(angle, box){
            ...
            Fucksvg.Animation.gravity(node1, {context:box, dx:0.3}).run().onStop(function(n){
				box.remove(node1);
				box.remove(node2);
				box.updateView();
				effect.stop();
			});
            Fucksvg.Animation.gravity(node2, {context:box, dx:-0.2}).run();
		}
	...
	function cycle(node, option){
		...
		var r = Fucksvg.util.getDistance(p1, p2)/2;
